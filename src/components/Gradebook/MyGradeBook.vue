  ...
  <div class=container>
    ...
    <table class="table table-striped table-bordered">
      <thead>
        <tr>
          <th>
            <button v-if="user.professor.id" class="btn btn-md btn-primary" @click="navigateToStudends()" style="min-width: 160px;">
                Add Student
              </button>
          </th>
          <th>Gradebook</th>
        ...
        <tr v-if="diary && diary.professor">
          ...
          <td>
            <ul style="list-style:none;">
              <li v-for="student in diary.students" :key="student.id">
    ...
    <div v-if="errorsList.length > 0" class="alert alert-danger"><p v-for="(error, index) in errorsList" :key="index">Message: {{ error }}</p>
    </div>
