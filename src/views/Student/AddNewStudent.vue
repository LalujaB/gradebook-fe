<template>
  <div class="container">
    <div class="form-group mt-4">
      ...
      <input type="text" class="form-control" id="firstName" name="firstName" v-model="newStudent.firstName"/>
    </div>

    <div class="form-group">
      <label for="lastName" class="text-left d-block">Last Name</label>
      <input type="text" class="form-control" id="lastName" name="lastName" v-model="newStudent.lastName"/>
    </div>
    ...
    <div>
      ...
      <div class="form-group" v-for="(url, index) in newStudent.url" :key="index">
        <div>
          <input :key="newStudent.url.id" autofocus v-model="url.url" placeholder="Image URL" class="form-control" :name="'image_' + newStudent.url[index].id" v-validate="{ required: true,  url , regex: /(?:(?:(?:\.jpg))|(?:(?:\.jpeg))|(?:(?:\.png)))/ }" required/>
