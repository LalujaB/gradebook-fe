  ...
  <div>
    ...
    <form class="container" @submit.prevent="handleForm">
      <div>
        ...
        <input type="text" id="firstName" class="ml-3" v-model="newProfessor.firstName" required minlength="2" maxlength="255"/>
      </div>
      <div class="mt-3">
        <label for="lastName mt-3">Last Name</label>
        <input type="text" id="lastName" class="ml-3" v-model="newProfessor.lastName" required minlength="2"/>
      </div>
      <div class="mt-3 mb-2">
        ...
        <div class="form-group mt-2" v-for="(url, index) in newProfessor.url" :key="index">
          <div>
            <input :key="newProfessor.url.id" autofocus v-model="url.url" placeholder="Image URL" class="form-control" :name="'image_' + newProfessor.url[index].id" v-validate="{ required: true,  url , regex: /(?:(?:(?:\.jpg))|(?:(?:\.jpeg))|(?:(?:\.png)))/ }" required/>
        ...
        <div class="form-group row diary mt-1">
          <label for="diary" class="form-control col-sm-1 mr-1">Diary</label>
          <select class="form-control col-sm-4" name="diary" id="diary" v-model="newProfessor.diary_id">
